...
//submitパラメータの値が存在するとき(登録ボタンをおしたとき)の処理
if(isset($_POST['submit'])){
    try {
        $pdo = new PDO($dsn, $user, $password);

        //動的に変わる値をプレースホルダ(:~~)に置き換えたINSERT文をあらかじめ用意する
        $sql = '
        ...
        //2.bindValue(パラメータID, $バインドする値, $PDOデータ型定数)メソッドを使って実際の値をプレースホルダにバインドする(割り当てる)
        //PARAM STRは文字列を意味する。
        ...
        //header()関数を使ってselect.phpにリダイレクトっさせる
...
<html lang='ja'>
    ...
    <body>
        ...
        <form action="insert.php" method="post">
            <div>
                <label for="user-name">お名前<span>[必須]</span></label>
                <input type="text" name="user_name" maxlength="60" required>

                <label for="user-furigana">ふりがな<span>[必須]</span></label>
