...
if (isset($_POST['submit'])){
    ...
    //SQL文を用意する（動的なのでprepareメソッド)
    $stmt_insert = $pdo->prepare($sql_insert);

    //bindValue()メソッドを使って実際の値をプレースホルダ(変数)にバインドする（割り当てる)
    ...
    //商品一覧ページにリダイレクトさせる(同時にmessageパラメータを渡す)
...
try {
    ...
    //SQL文の実行結果を配列で取得する
    //補足PDO::FETCH_COLUMNは１つのカラムの値を一次元配列(多次元ではない普通の配列)で取得する設定である
            ...
            <article class="registration">
                ...
                <form action="create.php" method="post" class="registration-form">
                    <div>
                        ...
                        <input type="text" name="product_name" max-length="50" required>
                        ...
                        <label for="vendor_code">仕入れコード</label>
